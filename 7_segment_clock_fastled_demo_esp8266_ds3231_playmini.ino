/*  7_segment_clock_fastled_demo_esp8266.ino, by Svein-Martin Holt, www.platelayer.com february 2024
 ...
 * To configure wifi, a phone can be used to access the ESP8266 to configure and connect to the network. The networkname and password is set using the phone.
 ...
 The software can be found here: https://github.com/smholt/7-segmentMaxiClock
...
==========
There are one buttons, a short push, change between clock mode and LED-strip demo mode.
A 5 second or longer push, let you change the volume for the bell, 0(No bell) to 30 in interval of 10.
Wait 5 seconds after last change, the new value is stored i EEPROM. On startup, the saved Volumevalue is used.
...
The password for the connections are: "password".
...
*/
#define PROG_NAME "7LED-clock"
#define BY_NAME "by platelayer"
#define PROG_VERSION "1.0"  // Set the program version number
  ...
  if (buttonState == HIGH && isButtonPressed) {
    ...
    if (pressDuration < mode2_time) {  // Less than 5 seconds
      if (mode == 0 || mode == 1) {
        ...
        // Toggle between 4 different values of a variable
        variableMode2 = (variableMode2 + 10) % 40;
      ...
      //mode = 0;
      //Serial.println("Mode changed to 0");
    }
  }
...
void setup() {
  ...
  EEPROM.begin(10);  // Allocate 10 bytes for EEPROM
  ...
  if (hour_t >= 8 && hour_t < 24) {
    ...
    if (minute_t == 0 && second_t < 2) {
      ...
      if (digitalRead(PLAYER_BUSY_PIN)) {  // play if ready(1)
        ...
        //myDFPlayer.playMp3Folder(hour_local);
        myMP3.playFromMP3Folder(hour_local);
      }
      /* else {
  ...
  if (mode != 2) {  //mode 2, change volume, dont update clock or display
    ...
    if (clock_mode) {
      ...
      if (timer1.timer(1000)) {  //change LED-strip every second
        ...
        getTime();
        if (timeClient.getSeconds() % 10 > 6) {  // display temperature for 4 seconds

          if (RTC_found) { displayGrads(); }  // display grads if RTC and temperature is avalable
...
void getTime() {
  ...
  */
  ...
  if (hour_t < 10) { display.print("0"); }  // Start at top-left corner
  display.print(hour_t);
  ...
  if (minute_t < 10) { display.print("0"); }
  display.println(minute_t);
  display.setTextSize(2);
